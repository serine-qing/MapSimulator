...
class Enemy extends BattleObject{
  ...
  public handleObstacleAvoidance(){
    if(this.unitVector.x === 0 && this.unitVector.y === 0){
  ...
  public update(delta: number){
    ...
    if(!this.isDisappear) this.move(delta);
  ...
  protected canMove(): boolean{
    if(this.isDisappear) return false;
    if(this.countdown.getCountdownTime("waiting") > 0) return false;
  ...
  protected updateCheckPoint(){
    // if(this.currentCheckPoint().type !== "MOVE") return;
    // if(!this.canMove()) return;
