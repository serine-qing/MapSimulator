<template>
  <div class="expand" v-if="hasData" :style="{ height: expandHeight + 'px' }">
    <div class="crisis-contract-page" >
    <!-- 全选按钮 -->
    <button class="toggle-all-btn" @click="toggleAll">
      {{ packIndicators.length > 0 && packIndicators.every(p => p.isActive) ? '全不选' : '全选' }}
    </button>
    ...
    <div class="detail-panel">
      ...
      <div class="detail-list">
        <div
          v-for="rune in selectedRunesData"
          :key="rune.runeId"
          class="rune-detail-item"
        >
          <div class="rune-detail-header">
            <span class="rune-detail-score">▲ {{ rune.score }}</span>
          </div>
          <div class="rune-detail-desc">
            <div v-for="(line, i) in rune.description" :key="i" v-html="line"></div>
          </div>
        </div>
      </div>
    </div>
    <div class="expand-toggle" @click="toggleExpand">
      {{ expandHeight === 400 ? '展开' : '收起' }}
    </div>
  </div>
</div>
</template>
...
import { getCCBMapData } from '@/api/stages'

// ==================== 伸缩控制 ====================
const expandHeight = ref(400)
const toggleExpand = () => {
  expandHeight.value = expandHeight.value === 400 ? window.innerHeight - 60.8 : 400
}
...
// 基础类型使用 ref
const hasData = ref(false);  //是否有ccb数据
...
const selectedRunesData = computed(() => getSelectedRunesData())
...
watch(() => mapId, async () => {
  if(mapId){
    hasData.value = false;
    mapData.value = null
    nodePosMap.value = new Map()
    roadPosMap.value = new Map()
    nodes.value = new Map()
    roads.value = []
    packDataMap.value = new Map()
...
async function loadData() {
  try {
    const apiData = await fetchCCData(mapId)
    if(!apiData || !apiData.nodes || !apiData.roads) return;
    hasData.value = true;
...
// 确认按钮
function onConfirm() {
  const runePack = getSelectedRunesData();
  const runes: RuneConfig[] = [];
  runePack.map(data => {
    runes.push(...data.packedRune.runes)
  })
  emit('confirm', runes)
}
...
function toggleAll() {
  if (packIndicators.value.length > 0) {
    ...
    // 无指标集：参考 handlePackClick 的模式
    const allSelected = displayNodes.value.every(n => !n.selectable || selectedRuneNodes.value.has(n.id))
    const newSelected = new Set<string>()
    ...
    if (!allSelected) {
      ...
      displayNodes.value.forEach(n => {
        if (n.type === 'rune') {
          const nodeData = nodes.value.get(n.id)
          if (nodeData) {
            const group = nodeData.mutualExclusionGroup || null
            if (!groupMap.has(group)) groupMap.set(group, [])
            groupMap.get(group)!.push(n.id)
          }
        }
      })
      ...
      while (changed) {
        ...
        while (kpChanged) {
          kpChanged = false
          nodes.value.forEach((nodeData: any, id: string) => {
            if (nodeData.nodeType !== 'KEYPOINT' || currentActives.has(id)) return
            const incomingRoads = roads.value.filter((r: any) =>
              r.end.type === 'NODE' && r.end.id === id
            )
            if (incomingRoads.some((r: any) =>
              r.start.type === 'NODE' && currentActives.has(r.start.id)
            )) {
              currentActives.add(id)
              kpChanged = true
...
.confirm-btn {
  ...
  box-shadow: 0 0 10px rgba(231, 76, 60, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.15);
  transition: all 0.2s;
  letter-spacing: 2px;
}

.confirm-btn:hover {
  background: linear-gradient(135deg, #e74c3c, #c41e3a);
  box-shadow: 0 0 20px rgba(231, 76, 60, 0.7), inset 0 1px 0 rgba(255, 255, 255, 0.2);
}
...
/* 滚动条样式 */
:deep(.detail-list)::-webkit-scrollbar,
:deep(.map-scroll-container)::-webkit-scrollbar {
  width: 6px;
  height: 6px;
}

:deep(.detail-list)::-webkit-scrollbar-track,
:deep(.map-scroll-container)::-webkit-scrollbar-track {
  background: transparent;
}

:deep(.detail-list)::-webkit-scrollbar-thumb,
:deep(.map-scroll-container)::-webkit-scrollbar-thumb {
  background: rgba(255, 255, 255, 0.4);
  border-radius: 3px;
}

:deep(.detail-list)::-webkit-scrollbar-thumb:hover,
:deep(.map-scroll-container)::-webkit-scrollbar-thumb:hover {
